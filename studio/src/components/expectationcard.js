...
function ExpectationCard({ Expectation , fileName }) {
  ...
  const isMoreHidden = isResultNested()
    ? getPartial_unexepected_list().length > 5
      ? true
      : false
    : true;
  ...
  const addIndexProperty = (unexpected_list) => {
    ...
    const unexpected_list_with_index = unexpected_list.map((object, index) => {
      return { ...object, 1: incrementedIndexList[index] };
    });
    return unexpected_list_with_index;
  };
  const getCsvHeaders = () =>
    Object.keys(getUnexpected_list()[0]).map((key) => {
      return {
        label: key === '1' ? 'Row' : key,
        key: key,
      };
  ...
  const getExpectationErrorMessage = () =>
    isResultNested()
      ? Expectation.results[0].expectation_config.meta.expectation_error_message
      : Expectation.expectation_config.meta.expectation_error_message;
  const getCleaningPdfLink = () =>
    isResultNested()
      ? Expectation.results[0].expectation_config.meta.cleaning_pdf_link
      : Expectation.expectation_config.meta.cleaning_pdf_link;
  ...
  return (
    <Card>
      <p>
        <b> {Expectation_name}</b>
      </p>
      <p>
        <span> Unwanted values count: </span> {getUnexpectedCount()}{' '}
      </p>
      <p>
        <span> {`Few Unwanted present in ${isResultNested() ? 'Column' : 'Row Number'}:`}</span>
      </p>
      <p>
        <span>
      ...
      <p>
        {' '}
        <Card style={{ backgroundColor: '#feecf0' }}>
          {getExpectationErrorMessage()}
          <Button
            style={{ paddingLeft: '10px' }}
            target="_blank"
            href={getCleaningPdfLink()}
      ...
      >
        {!isResultNested() ? (
          <>
            <div style={{ padding: '12px 24px 0px 0px', float: 'right', clear: 'both' }}>
              {' '}
              <Button type="primary">
                <CSVLink
                  headers={getCsvHeaders()}
                  filename={`${Expectation_name}.csv`}
                  data={unexpectedList}
                >
